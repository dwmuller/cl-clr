...
(defmethod on-create-device ((this matrices) sender e)
  (with-slots (device vertex-buffer) this
    (setf vertex-buffer
          (new '?VertexBuffer
               (to-clr-type-object '?CustomVertex+PositionColored)
               3
               device
               (?Usage.None)
...
(defmethod render ((this matrices))
  (with-slots (device vertex-buffer pause) this
    ...
    (when pause
      (return-from render))
    (?Clear device (?ClearFlags.Target) (?Color.Blue) 1.0f0 0)
    (?BeginScene device)
    (setup-matrices this)
    ...
    (setf (?VertexFormat device)
          (?CustomVertex+PositionColored.Format))
    (?DrawPrimitives device (?PrimitiveType.TriangleList) 0 1)
    (?EndScene device)
    (?Present device)))

(defmethod setup-matrices ((this matrices))
  (with-slots (device) this
    (let* ((itime (rem (?Environment.TickCount) 1000))
           (fangle (/ (* itime 2.0f0 pi) 1000.0f0)))
      (setf (?World (?Transform device)) (?Matrix.RotationY fangle)
            ...
            (?Projection (?Transform device))
              (?Matrix.PerspectiveFovLH (/ pi 4) 1.0f0 1.0f0 100.0f0)))))
